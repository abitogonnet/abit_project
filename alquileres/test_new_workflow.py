...
class NewWorkflowTests(TestCase):
    ...
    def test_ver_alquileres_no_muestra_buscador_y_siempre_lista_todos(self):
        primero = self.alquiler(cliente_nombre="María Soto")
        segundo = self.alquiler(cliente_nombre="Pedro López")
        response = self.client.get(reverse("alquileres:ver"), {"buscar": primero.id})
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, "Buscar cualquier alquiler")
        self.assertNotContains(response, 'name="buscar"', html=False)
